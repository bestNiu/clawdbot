...
本页涵盖**LLM/模型提供者**（不是像 WhatsApp/Telegram 这样的聊天通道）。
...
- CLI 助手：`clawdbot onboard`、`clawdbot models list`、`clawdbot models set <provider/model>`。

## 内置提供者（pi-ai 目录）

Clawdbot 附带 pi‑ai 目录。这些提供者**不需要**`models.providers` 配置；只需设置认证 + 选择模型。
...
- 提供者：`openai-codex`
- 认证：OAuth 或 Codex CLI（`~/.codex/auth.json`）
- 示例模型：`openai-codex/gpt-5.2`
- CLI：`clawdbot onboard --auth-choice openai-codex` 或 `codex-cli`
...
- 认证：Vertex 使用 gcloud ADC；Antigravity/Gemini CLI 使用各自的认证流
...
- Gemini CLI OAuth 作为捆绑插件提供（`google-gemini-cli-auth`，默认禁用）。
  ...
  - 注意：你**不**将客户端 ID 或密钥粘贴到 `clawdbot.json` 中。CLI 登录流将令牌存储在网关主机上的认证配置文件中。
...
使用 `models.providers`（或 `models.json`）添加**自定义**提供者或 OpenAI/Anthropic‑兼容代理。
...
有关设置详细信息和说明，请参见 [/providers/qwen](/providers/qwen)。
...
当在 `http://127.0.0.1:11434/v1` 本地运行时，Ollama 会自动检测。有关模型推荐和自定义配置，请参见 [/providers/ollama](/providers/ollama)。
...
另请参见：[/gateway/configuration](/gateway/configuration) 以获取完整配置示例。
