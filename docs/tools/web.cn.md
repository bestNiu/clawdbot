...
有关提供者特定详细信息，请参见 [Brave Search 设置](/brave-search）和 [Perplexity Sonar](/perplexity）。
...
3) 运行 `clawdbot configure --section web` 将密钥存储在配置中（推荐），或在环境中设置 `BRAVE_API_KEY`。

Brave 提供免费层加付费计划；检查 Brave API 门户以获取当前限制和定价。

### 在哪里设置密钥（推荐）

**推荐：** 运行 `clawdbot configure --section web`。它将密钥存储在 `~/.clawdbot/clawdbot.json` 下的 `tools.web.search.apiKey` 中。

**环境替代：** 在 Gateway 进程环境中设置 `BRAVE_API_KEY`。对于网关安装，将其放在 `~/.clawdbot/.env`（或你的服务环境）中。参见 [环境变量](/help/faq#how-does-clawdbot-load-environment-variables）。

## 使用 Perplexity（直接或通过 OpenRouter）

Perplexity Sonar 模型具有内置的网络搜索功能，并返回带有引用的 AI 综合答案。你可以通过 OpenRouter 使用它们（无需信用卡 - 支持加密/预付费）。
  ...
  tools: {
    web: {
      search: {
        ...
        provider: "perplexity",
        perplexity: {
          // API 密钥（如果设置了 OPENROUTER_API_KEY 或 PERPLEXITY_API_KEY 则为可选）
          apiKey: "sk-or-v1-...",
          // 基础 URL（如果省略则为键感知默认值）
...
**环境替代：** 在 Gateway 环境中设置 `OPENROUTER_API_KEY` 或 `PERPLEXITY_API_KEY`。对于网关安装，将其放在 `~/.clawdbot/.env` 中。

如果未设置基础 URL，Clawdbot 根据 API 密钥源选择默认值：
...
| 模型 | 描述 | 最适合 |
|-------|-------------|----------|
| `perplexity/sonar` | 快速 Q&A 与网络搜索 | 快速查找 |
| `perplexity/sonar-pro`（默认） | 多步骤推理与网络搜索 | 复杂问题 |
| `perplexity/sonar-reasoning-pro` | 链式思考分析 | 深入研究 |
...
注意：
- `web_fetch` 首先使用 Readability（主内容提取），然后使用 Firecrawl（如果配置）。如果两者都失败，工具返回错误。
- Firecrawl 请求使用反机器人规避模式，默认缓存结果。
